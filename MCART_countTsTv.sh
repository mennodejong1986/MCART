...
# An alternative could be to use vcftools --TsTv-summary or --FILTER-summary, but I found these tools output results which deviate from expectations
# vcftools seems to filter out some sites, resulted in a (slightly) different estimate

####################################################
MYVCFGZ=BphyP11551205L2_toBmus-ref_filtered.vcf.gz
####################################################
...
echo " "
echo "Results have been stored in file 'tstv_counts.txt'",sep="\n")
echo "Output table:"
cat tstv_counts.txt